...
class ParlaImagesPipeline(ImagesPipeline):
    members = {}
    def __init__(self, *args, **kwargs):
        super(ParlaImagesPipeline, self).__init__(*args, **kwargs)
...
class ParlaparserPipeline(object):
    ...
    people_mandate_start_time = datetime(day=27, month=2, year=2019)
    ...
    def __init__(self):
        ...
        for pg in paries:
            ...
            if pg['classification'] == 'party' or pg['classification'] == 'gov' or pg['classification'] == 'pg':
        ...
        print('pipeline get sessions with speeches')
        sessions_with_speeches = getDataFromPagerApiDRF(API_URL + 'sessions/sessions_with_speeches/')
        self.sessions_with_speeches = [session['gov_id'] for session in sessions_with_speeches]
        ...
        try:
            people_items = requests.get(API_URL + 'getPeopleMembershipsOfMembers').json()
        except:
            people_items = []
        self.memberships = items
        self.memberships.update(people_items)
