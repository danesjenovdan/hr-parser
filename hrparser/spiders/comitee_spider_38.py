...
# [GCC 9.3.0]
# Embedded file name: /home/tomaz/dev/DJND/hrparser/hrparser/spiders/comitee_spider.py
# Compiled at: 2020-02-13 11:48:39
...
class ComiteeSpider(scrapy.Spider):
    ...
    custom_settings = {'ITEM_PIPELINES': {'hrparser.pipelines.HrparserPipeline': 1}}
    ...
    def __init__(self, parse_memberships=False, **kwargs):
        self.start_urls = [
         'https://sabor.hr/hr/radna-tijela/odbori-i-povjerenstva']
    ...
    def parse(self, response):
        for i in response.css('.item-list li'):
            ...
            yield scrapy.Request(url=('https://sabor.hr' + link.strip()),
        ...
        if self.parse_memberships:
            ...
            for datum in drupal_settings['datumi']:
                yield scrapy.Request(url=self.calendar_link % (datum, wb_id),
                  ...
                  meta={'wb_name':response.meta['wb_name'], 
                 'wb_id':wb_id},
                  priority=5)

            data = {'view_name':'sabor_eva',
    ...
    def parse_single_session_from_calendar(self, response):
        ...
        try:
            ...
            yield {'type':'calendar_session', 
             'datetime_utc':datetime_utc, 
             'session_name':session_name, 
             'agenda_items':agenda_items, 
             'wb_title':response.meta['wb_name']}
# okay decompiling comitee_spider.cpython-38.pyc
